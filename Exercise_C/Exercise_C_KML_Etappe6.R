...
extract_placemark_details <- function(placemark) {
  ...
  # Umwandlung der Koordinaten in ein sf-Objekt
  coordinates_sf <- st_as_sf(data.frame(Longitude = longitudes, Latitude = latitudes), 
                             coords = c("Longitude", "Latitude"), 
                             crs = 4326) # WGS84
  
  # Transformation in das Schweizer Koordinatensystem LV95 (EPSG:2056)
  coordinates_lv95 <- st_transform(coordinates_sf, 2056)
  
  # Extraktion der transformierten Koordinaten
...
# Daten mit Distanz 0.0 km entfernen
filtered_df <- coord_df %>% filter(Distance_km != 0)
